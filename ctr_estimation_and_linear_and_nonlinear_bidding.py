...
4. Ohter linear bidding models based on the stacked model's pCTR

This is later useful for the multi-agent setting.

The bids are evaluated as before and the model is calibrated as before:

                        bid = base_bid* pCTR'/avgCTR
                        
where pCTR' are the stacked model's pCTR. These have scales 100-1000 times higher
than the other pCTRs as the stacked learner has probability predictions as input rather
than a sparse binary-valued matrix. Hence, the base bid grid must be adjusted accordingly

Also, upon experimentation, the linear bidding was limited to the pctrs within of the
20th and 95th percentiles of pctrs.

'''



with open('stacked_pctr_validation.csv') as csv_file: 
    s_p_ctr = pd.read_csv(csv_file)


p_ctr = s_p_ctr['0'].values
new = p_ctr.copy()
lower = 20
upper = 95
new[new<=np.percentile(np.sort(new),lower)]=0
new[new>np.percentile(np.sort(new),upper)]=0
p_ctr = new

avg_ctr = sum(train['click']==1)/len(train)
budget = 6250
data_payprice = np.array(validation['payprice'].as_matrix()).reshape(-1,1) + 300
base_bid_grid = np.linspace(50, 70, 100)/100
...
# obtain bids on test set  
with open('stacked_pctr.csv') as csv_file: 
    s_p_ctr_test = pd.read_csv(csv_file)
    
tst_pctr = s_p_ctr_test['0'].values
        
temp = tst_pctr.copy()
temp[temp<=np.percentile(np.sort(temp),lower)]=0
temp[temp>np.percentile(np.sort(temp),upper)]=0
p_ctr_test = temp  
        
        
bids = (optimal_base_bid_no_clicks*p_ctr_test/avg_ctr).reshape(-1, 1)
pd.DataFrame(bids).to_csv("test_bids.csv")
